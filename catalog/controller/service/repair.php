...
class ControllerServiceRepair extends Controller {
	...
	public function index() {
		...
		$data['heading_title'] = "Nt-phones repair services";

		$this->load->model('service/repair');

		$categories = $this->model_service_repair->getCategories();
		...
		foreach($categories as $result){
			...
			$data['categories'][] = array(
				'name' => $result['name'],
				'image' => $image,
				'href' => $this->url->link('service/repair/category', 'path='.$result['category_id'])
			);
		}
		...
		if ($category_info) {
			...
			foreach ($results as $result) {
				...
				$data['products'][] = array(
					...
					'href'        => $this->url->link('service/repair/product', 'path=' . $this->request->get['path'] . '&product_id=' . $result['product_id'] . $url)
				);
			}
			...
			$this->response->setOutput($this->load->view('service/repair_category', $data));
		} else {
			$url = '';
	...
	public function product() {
		...
		if ($product_info) {
			...
			$data['heading_title'] = " Repair ".$product_info['name'];
			...
			$results = $this->model_service_repair_rate->getProductRates($product_id);
			...
			$this->response->setOutput($this->load->view('service/product', $data));
		} else {
			$url = '';
