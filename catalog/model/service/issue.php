...
class ModelServiceIssue extends Model {
	...
	public function getProductIssue($product_id,$issue_id) {
		
		$sql = "SELECT i.issue_id,r.product_id, r.price, i.title,i.image,i.description FROM " . DB_PREFIX . "repair_rate r JOIN " . DB_PREFIX . "issue i ON r.issue_id = i.issue_id where price > 0 AND r.product_id =".(int)$product_id." AND r.issue_id=".(int)$issue_id;
